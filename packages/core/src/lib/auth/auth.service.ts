...
export class AuthService extends SocialAuthService {
	...
	async sendWorkspaceSigninCode(
		...
		try {
			...
			if (IS_DEMO) {
				const demoEmployeeEmail = environment.demoCredentialConfig?.employeeEmail || 'employee@infiria.com';
				const demoAdminEmail = environment.demoCredentialConfig?.adminEmail || 'local.admin@infiria.com';
	 ...
	 * For SUPER_ADMIN users, returns all workspaces in the system.
	 * For regular users, returns only workspaces where they have the same email.
	 *
	 * @param includeTeams Flag indicating whether to include team information in the response.
